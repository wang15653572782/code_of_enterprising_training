...
def migration_all_date(areaname, classname, no, direction):  # 定义生成不同时期，不同城市，不同迁徙方向
    ...
    for date1 in range(20190121, 20190132):  # 一月份
        datelist.append(date1)
    for date2 in range(20190201, 20190229):  # 二月份
        datelist.append(date2)
    for date3 in range(20190301, 20190302):  # 二月份
    ...
    for date in datelist:  # 遍历所有日期
        ...
        print(url)
        response = requests.get(url, timeout=5)  # 发出请求并json化处理
        time.sleep(1)
